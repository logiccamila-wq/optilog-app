 ...
 */
export const manualInvoiceOCR = functions.https.onCall(async (data, context) => {
  // Verificar autenticação
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Usuário deve estar autenticado para usar esta função'
    );
  }
  ...
  if (!companyId || !fileUrl) {
    throw new functions.https.HttpsError(
      'invalid-argument',
      'Parâmetros companyId e fileUrl são obrigatórios'
    );
  }
